...
export async function ProductNav() {
  ...
  return <><a className="skip-link" href="#main-content">Skip to main content</a><header className="product-header"><Link href="/" className="product-brand" aria-label="Pnyx home"><Image src="/brand/pnyx-mark.svg" alt="" width={32} height={32} priority/><span>PNYX</span></Link><form className="shell-search" action="/explore" method="get"><label><span className="sr-only">Search Pnyx</span><input name="q" placeholder="Search Pnyx" /></label></form><Link href="/explore" className="shell-search-trigger" aria-label="Search Pnyx"><span aria-hidden="true">⌕</span></Link><div className="product-actions"><ThemeToggle/><SessionNav user={user} unreadCount={unreadCount}/></div></header><AppNavigation/><aside className="shell-context" aria-label="Contextual navigation"><p className="shell-label">Keep exploring</p><Link href="/knowledge"><strong>Knowledge library</strong><span>Guides, answers, research, and sources.</span></Link><Link href="/explore"><strong>Find a signal</strong><span>Search public technology conversations.</span></Link><Link href="/ai"><strong>AI assistance</strong><span>Summarize and connect selected discussions.</span></Link></aside></>;
}
