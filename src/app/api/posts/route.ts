...
export async function GET(request: Request) {
  const url = new URL(request.url); const page = pageSchema.parse(url.searchParams.get("page") ?? 1); const limit = 12; const requestedFeed = url.searchParams.get("feed") ?? "latest"; const feed = ["latest", "following", "trending"].includes(requestedFeed) ? requestedFeed : "latest"; const spaceSlug = url.searchParams.get("space"); const session = await auth();
  ...
  const posts = await prisma.post.findMany({ where: { isDeleted: false, isDraft: false, ...(spaceSlug ? { space: { slug: spaceSlug } } : {}), authorId: blocked.length ? { notIn: blocked.map((b) => b.blockedId) } : undefined, spaceId: muted.length ? { notIn: muted.map((m) => m.spaceId) } : undefined, ...(feed === "following" ? { authorId: { in: following.map((f) => f.followingId), ...(blocked.length ? { notIn: blocked.map((b) => b.blockedId) } : {}) } } : {}) }, include: { author: { select: { username: true, displayName: true, avatarUrl: true } }, space: { select: { slug: true, name: true } }, reactions: { where: { userId: session?.user?.id ?? "__anonymous__" }, select: { userId: true } }, bookmarks: { where: { userId: session?.user?.id ?? "__anonymous__" }, select: { userId: true } }, _count: { select: { replies: true, reactions: true, bookmarks: true } } }, orderBy: feed === "trending" ? [{ reactions: { _count: "desc" } }, { replies: { _count: "desc" } }, { createdAt: "desc" }] : { createdAt: "desc" }, skip: (page - 1) * limit, take: limit });
  return NextResponse.json({ posts: posts.map((post) => ({ ...post, viewer: { reacted: Boolean(post.reactions?.length), bookmarked: Boolean(post.bookmarks?.length) }, reactions: undefined, bookmarks: undefined })), page, hasMore: posts.length === limit });
}
